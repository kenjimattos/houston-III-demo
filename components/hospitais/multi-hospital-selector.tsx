...
interface MultiHospitalSelectorProps {
  value: string[] // Array de IDs dos hospitais selecionados
...
export function MultiHospitalSelector({
  value = [],
  onChange,
  onHospitalCreated,
  className,
      ...
      try {
        ...
        if (savedPinned) {
          ...
          if (pinnedIds.length > 0) {
            ...
            if (data) {
              // Converter dados do banco para interface Hospital
              const convertedData = data.map((h: any) => ({
                hospital_id: h.id,
              ...
              const orderedPinned = pinnedIds
                .map(id => convertedData.find(h => h.hospital_id === id))
  ...
  // Carregar hospitais selecionados quando value muda
  useEffect(() => {
    const loadSelectedHospitals = async () => {
      if (value.length > 0) {
        try {
          const supabase = getSupabaseClient()
          ...
          if (data) {
            ...
            setSelectedHospitals(convertedData)
          }
        } catch (error) {
          console.error('Erro ao carregar hospitais selecionados:', error)
      ...
      if (data.length < 5) {
        ...
        if (words.length > 0) {
          ...
          if (additionalData) {
            ...
            const combined = [...data]
            additionalData.forEach(hospital => {
              if (!combined.find(h => h.id === hospital.id)) {
                combined.push(hospital)
              }
            })

            // Converter dados do banco para interface Hospital
            const convertedCombined = combined.map((h: any) => ({
              hospital_id: h.id,
              hospital_nome: h.nome,
              hospital_cnpj: h.cnpj,
              hospital_endereco: h.endereco_formatado,
            ...
            setHospitais(convertedCombined)
            return
          }
        }
      }
  ...
  // Verificar se hospital já está selecionado
  const isSelected = (hospitalId: string) => {
    return value.includes(hospitalId)
  }

  // Handler para seleção/desseleção
  const toggleSelection = (hospital: Hospital) => {
    if (isSelected(hospital.hospital_id)) {
      // Remover da seleção
      const newSelection = value.filter(id => id !== hospital.hospital_id)
      ...
      if (maxSelections && value.length >= maxSelections) {
        toast({
          ...
          description: `Você pode selecionar no máximo ${maxSelections} hospitais.`,
          variant: "destructive"
        })
        return
      }
      ...
      // Adicionar à seleção
      const newSelection = [...value, hospital.hospital_id]
      onChange(newSelection)
    }
  }
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              className={cn(
                ...
                selectedHospitals.length === 0 && "text-muted-foreground",
                className
              )}
            >
              {selectedHospitals.length > 0 ? (
                <span className="flex items-center gap-2">
                  <span>{selectedHospitals.length} hospital{selectedHospitals.length > 1 ? 'is' : ''} selecionado{selectedHospitals.length > 1 ? 's' : ''}</span>
                </span>
              ) : (
                <span className="flex items-center gap-2">
                  ...
                  <div className="border-b">
                    ...
                    <div className="p-1">
                      {pinnedHospitals.map((hospital) => {
                        const selected = isSelected(hospital.hospital_id)
                        return (
                          <div key={hospital.hospital_id} className="group relative">
                            <button
                              onClick={() => toggleSelection(hospital)}
                              className={cn(
